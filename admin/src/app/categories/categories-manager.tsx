...
export function CategoriesManager({ initialRows }: { initialRows: Category[] }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const row = await api.post<Category>(
        "/categories",
        {
          name: form.name,
    ...
    <>
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[var(--muted)]">Manage shop navigation groups.</p>
        </div>
      ...
      {open && (
        <form onSubmit={submit} className="surface mb-6 grid gap-3 p-5 md:grid-cols-2">
          ...
          <button className="rounded-md bg-[var(--accent)] px-4 py-3 font-semibold text-[var(--accent-ink)] md:col-span-2">Save category</button>
        </form>
      )}
      <DataTable rows={rows} columns={[{ key: "name", label: "Name" }, { key: "slug", label: "Slug" }, { key: "sortOrder", label: "Sort" }, { key: "isActive", label: "Active", render: (row) => (row.isActive ? "Yes" : "No") }]} />
    </>
  );
}
